...
def train_classifier():
    ...
    for epoch in range(EPOCHS):
        ...
        for img,label,_,_ in loader:
            ...
            total+=label.size(0)
        wandb.log({"cls/loss":total_loss/len(loader),"cls/acc":correct/total,"epoch":epoch+1})
...
def train_localizer():
    ...
    for epoch in range(EPOCHS):
        ...
        for img,_,bbox,_ in loader:
            ...
            opt.step()
            total_loss+=loss.item()
        wandb.log({"loc/loss":total_loss/len(loader),"epoch":epoch+1})
...
def train_unet():
    ...
    for epoch in range(EPOCHS):
        ...
        for img,_,_,mask in loader:
            ...
            opt.step()
            total_loss+=loss.item()
        wandb.log({"seg/loss":total_loss/len(loader),"epoch":epoch+1})
